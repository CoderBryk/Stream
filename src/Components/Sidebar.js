...
const Sidebar = ({ selectedCategory, setSelectedCategory}) => {
  ...
  return (
    ...
    { categories.map((category) => (
     <button onClick ={() => setSelectedCategory(category.name)} className='sidebar_btn' style={{background: category.name === selectedCategory && '#FC1503', color: '#fff'}} key={ category.name }>
